...
func main{
    ...
    let (res: felt, new_state : State) = next(s);
    serialize_word(res);
    
    let (res: felt, new_state : State) = next(new_state);
    serialize_word(res);
    return ();
}

func next{bitwise_ptr: BitwiseBuiltin*, pedersen_ptr: HashBuiltin*, range_check_ptr}(s: State) -> (
    rnd: felt, new_state : State
) {
    alloc_locals;
