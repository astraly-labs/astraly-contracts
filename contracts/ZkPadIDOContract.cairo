...
from contracts.utils.ZkPadUtils import get_is_equal, uint256_mul_checked
from starkware.starknet.common.syscalls import (get_block_timestamp)
from openzeppelin.utils.constants import FALSE, TRUE
from openzeppelin.token.erc20.interfaces.IERC20 import IERC20
from starkware.cairo.common.uint256 import (
    Uint256, uint256_eq, uint256_add, uint256_mul, uint256_le, uint256_lt, uint256_check, uint256_unsigned_div_rem)
...
func populate_vesting_params_rec{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*,range_check_ptr}(
    ...
    let (local percent_sum0 : Uint256, is_overflow) = uint256_add(_percents_sum, percent0)
    with_attr error_message("ZkPadIDOContract::populate_vesting_params_rec percent sum overflow"):
...
func set_sale_params{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*,range_check_ptr} (
    ...
    with_attr error_message("ZkPadIDOContract::set_sale_params IDO Token price must be greater than zero"):
        let (token_price_check : felt) = uint256_lt(Uint256(0,0), _token_price)
        assert token_price_check = TRUE
    end
    with_attr error_message("ZkPadIDOContract::set_sale_params Number of IDO Tokens to sell must be greater than zero"):
    ...
    with_attr error_message("ZkPadIDOContract::set_sale_params portion vesting percision should be at least 100"):
        let (vesting_precision_check : felt) = uint256_le(Uint256(100,0), _portion_vesting_precision)
...
func register_user{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(amount: Uint256, account: felt) -> (res: felt):
    ...
    if is_user_reg == 0:
        ...
        let (local registrants_sum : Uint256, is_overflow) = uint256_add(the_reg.number_of_registrants, Uint256(low=1, high=0))
        with_attr error_message("ZkPadIDOContract::register_user registrants num overflow"):
            assert is_overflow = 0
        end
        let upd_reg = Registration(
    ...
    let (local winning_tickets_sum : Uint256, is_overflow) = uint256_add(current_winning, new_winning)
    with_attr error_message("ZkPadIDOContract::register_user winning tickets sum overflow"):
        assert is_overflow = 0
    end
    user_to_winning_lottery_tickets.write(account, winning_tickets_sum)

    let (local total_winning_tickets_sum : Uint256, is_overflow) = uint256_add(the_sale.total_winning_tickets, new_winning)
    with_attr error_message("ZkPadIDOContract::register_user total winning tickets sum overflow"):
        assert is_overflow = 0
    end    
    let upd_sale = Sale(
...
func get_adjusted_amount{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(_amount : Uint256, _cap : Uint256) -> (res : Uint256):
...
func calculate_allocation{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*,range_check_ptr}():
    ...
    let (the_allocation, _) = uint256_unsigned_div_rem(the_sale.amount_of_tokens_to_sell, the_sale.total_winning_tickets)
...
func draw_winning_tickets{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*,range_check_ptr}(tickets_burnt: Uint256, account: felt) -> (res: Uint256):
    alloc_locals
    let (rnd) = get_random_number()
    const max_denominator = 18446744073709551615 #0xffffffffffffffff
    let (max_uint) = _felt_to_uint(max_denominator)
    let (num : Uint256) = uint256_mul_checked(tickets_burnt, rnd)
    let (winning, _) = uint256_unsigned_div_rem(num, max_uint) 
    return (res=winning)
end

func get_random_number{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*,range_check_ptr}() -> (rnd : Uint256):
...
func participate{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*,range_check_ptr}(
    ...
    let (max_tokens_to_purchase : Uint256) = uint256_mul_checked(winning_tickets, the_alloc)
    let (number_of_tokens_byuing, _) = uint256_unsigned_div_rem(amount_paid, the_sale.token_price)
    ...
    #Updates
    let (local total_tokens_sum : Uint256, is_overflow) = uint256_add(the_sale.total_tokens_sold, number_of_tokens_byuing)
    with_attr error_message("ZkPadIDOContract::participate total tokens sum overflow"):
        assert is_overflow = 0
    end
    let (local total_raised_sum : Uint256, is_overflow) = uint256_add(the_sale.total_raised, amount_paid)
    with_attr error_message("ZkPadIDOContract::participate total raised sum overflow"):
        assert is_overflow = 0
    end    
    let upd_sale = Sale(
        token = the_sale.token,
    ...
    let (local nbr_participants_sum : Uint256, is_overflow) = uint256_add(nbr_participants, Uint256(low=1, high=0))
    with_attr error_message("ZkPadIDOContract::participate number of participants sum overflow"):
        assert is_overflow = 0
    end
    number_of_participants.write(nbr_participants_sum)

    let (factory_address) = ido_factory_contract_address.read()
    let (pmt_token_addr) = IZKPadIDOFactory.get_payment_token_address(contract_address=factory_address)
    with_attr error_message("ZkPadIDOContract::participate Payment token address not set"):
        assert_not_zero(pmt_token_addr)
    end
    let (pmt_success : felt) = IERC20.transferFrom(pmt_token_addr, account, address_this, amount_paid)
