...
func remove_lock_time_bonus{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (value_multiplied : Uint256) = SafeUint256.mul(shares, Uint256(low=730, high=0))
...
func check_enough_underlying_balance{
    ...
    if not_enough_balance_in_contract == TRUE:
        ...
        let (float_missing_for_withdrawal : Uint256) = SafeUint256.sub_lt(underlying_amount, total_float)

        let (withdraw_amount_required : Uint256) = SafeUint256.add(float_missing_for_target, float_missing_for_withdrawal)
...
func pull_from_withdrawal_stack{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    if enough_balance_in_strategy == TRUE:
        ...
        let (remaining_amount_in_strategy_balance : Uint256) = SafeUint256.sub_le(strategy_details.balance, amount_to_withdraw)
        strategy_data.write(strategies[index], StrategyData(TRUE, remaining_amount_in_strategy_balance))
        return (Uint256(0, 0))
    else:
        let (remaining_amount : Uint256) = SafeUint256.sub_le(amount_to_withdraw, strategy_details.balance)
