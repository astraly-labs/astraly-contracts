...
namespace Referral {
    func record_referral{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (referrer_) = Referral_referrer.read(user);
        with_attr error_message("record_referral::{user} is already referred") {
            assert referrer_ = 0;
        }
        with_attr error_message("record_referral::self referral is not allowed") {
            assert_not_equal(user, referrer);
        }
