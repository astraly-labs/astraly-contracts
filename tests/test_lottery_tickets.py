...
accountSigner = MockSigner(123456789987654321)
...
async def erc1155_init(contract_defs):
    ...
    account1 = await starknet.deploy(
        ...
        constructor_calldata=[accountSigner.public_key]
    )
    ...
    await accountSigner.send_transaction(account1, zk_pad_stake_proxy.contract_address, "initializer", [
    ...
    await accountSigner.send_transaction(account1, factory.contract_address, "set_task_address", [task.contract_address])
    root = generate_merkle_root(list(map(lambda x: x[0], MERKLE_INFO)))
    await accountSigner.send_transaction(account1, factory.contract_address, "set_merkle_root", [root, 0])

    await accountSigner.send_transaction(account1, factory.contract_address, 'set_lottery_ticket_contract_address', [erc1155.contract_address])
    await accountSigner.send_transaction(account1, factory.contract_address, 'set_random_number_generator_address', [rnd_nbr_gen.contract_address])
...
async def erc1155_minted_init(contract_defs, erc1155_init):
    ...
    await accountSigner.send_transaction(
    ...
    # Create mock IDO
    await accountSigner.send_transaction(owner, factory.contract_address, "create_ido", [owner.contract_address])
...
async def full_init(contract_defs, erc1155_init):
    ...
    tx = await accountSigner.send_transaction(
        owner,
    ...
    root = generate_merkle_root(list(map(lambda x: x[0], MERKLE_INFO)))
    await accountSigner.send_transaction(owner, factory.contract_address, "set_merkle_root", [root, 0])
    # print("ROOT", root)
...
async def test_set_approval_for_all_non_boolean(erc1155_factory):
    ...
    await assert_revert(accountSigner.send_transaction(
        account, erc1155.contract_address, 'setApprovalForAll',
        [operator, approval]
    ))
...
async def test_mint_to_zero_address(erc1155_factory):
    ...
    await assert_revert(
        accountSigner.send_transaction(
            owner, erc1155.contract_address, 'mint',
            [
                recipient,  # to
                *token_id,
...
async def test_safe_batch_transfer_from(erc1155_minted_factory):
    ...
    execution_info = await accountSigner.send_transaction(
...
async def test_claim_success(full_factory):
    ...
    # Claim tickets
    await accountSigner.send_transaction(owner, erc1155.contract_address, 'claimLotteryTickets', [*IDO_ID, 0])
...
async def test_claim_twice_fail(full_factory):
    ...
    await assert_revert(accountSigner.send_transaction(owner, erc1155.contract_address, 'claimLotteryTickets', [
...
async def test_claim_twice_success(full_factory):
    ...
    await accountSigner.send_transaction(owner, erc1155.contract_address, 'claimLotteryTickets', [*IDO_ID1, 0])

    # Attempt to claim again for IDO 1
    await accountSigner.send_transaction(owner, erc1155.contract_address, 'claimLotteryTickets', [
        *IDO_ID2, 0])
...
async def test_claim_no_tickets(full_factory):
    ...
    await assert_revert(accountSigner.send_transaction(user, erc1155.contract_address, 'claimLotteryTickets', [
...
async def test_kyc(erc1155_factory):
    ...
    sig = accountSigner.signer.sign(message)
    print(sig)
    await accountSigner.send_transaction(owner, erc1155.contract_address, 'checkKYCSignature', [len(sig), *sig])
