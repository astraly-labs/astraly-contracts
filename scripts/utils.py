...
def generate_proof_helper(level, index, proof, cached_level):
    if len(level) == 1:
        return proof
    if len(level) % 2 != 0:
        level.append(0)

    # if not level in cached_level:
    #     cached_level[level] = get_next_level(level)
    var = level in cached_level["1"]
    if not var:
...
def generate_merkle_proof(values, index, cached_level):
    l = generate_proof_helper(values, index, [], cached_level)
